...
class HomogeneousCoordinatesPoint(np.ndarray):
    def __new__(cls, *coords):
        invalid_shape_message = "Invalid input shape:\n" \
            "Expected a 2D np.array of shape ({},N) in non-homogenous coordinates\n" \
    ...
    # /!\ iter conflicts with numpy array getitem.
    # def __iter__(self):
    #     return (self.__class__(self[:,i].reshape((-1,1))) for i in range(self.shape[1]))
    ...
    get_coord = lambda self, i: self[i,0] if self.shape[1] == 1 else self[i]
    set_coord = lambda self, i, value: self.__setitem__((i,0), value) if self.shape[1] == 1 else self.__setitem__((i), value)
...
class Calib():
    ...
    def distort(self, point2D: Point2D):
        if np.any(self.kc):
            ...
            dx = np.array([
                2*tan1*point2D[0]*point2D[1] + tan2*(r2 + 2*point2D[0]*point2D[0]),
                2*tan2*point2D[0]*point2D[1] + tan1*(r2 + 2*point2D[1]*point2D[1])
            ])
    ...
    def rectify(self, point2D: Point2D):
        if np.any(self.kc):
            ...
            point2D = (point2D - Point2D(dx))/delta
    ...
    def compute_length2D(self, length3D: int, point3D: Point3D):
        ...
        point3D_c[0] += length3D # add the 3D length to one of the componant
